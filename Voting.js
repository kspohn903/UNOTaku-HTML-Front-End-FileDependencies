...
let goNegVote = (btn) => {
 ...
 let ajaxGetSingleAnime = (el) => { /* Get single anime info */
  if (el.value > 0) {
	  ...
	  !!! NOTE: !!! - modify/deconstruct original DB.php, and front-end inject titles into a MySQL DB instead. 
	  Let any titles obtained render all list items on table creation with form-entry being successful. */
...
let confirmSubmit = () => {
	if ($("#XMLNumber").val() == "") {
		...
		return false;
	} else if ($("#XMLNumber").val() == -1 || $("#XMLNumber").val() > 0) {
		let confirmationMsg = ($("#XMLNumber").val() == -1)? "Submit this new anime?": 
		("Submit edits to '" + $("#XMLName").val() + "', previously '" + $("#editAnime option:selected'").text() +"'?");
		return confirm(confirmationMsg);
	} else {
		alert("No idea what you just did...");
		return false;
	} /*end of confirmSubmit method...*/ };
